...
class FeishuClient:
        ...
        **kwargs,
        ...
        async with httpx.AsyncClient() as client:
            ...
            if result.get("code") != 0:
                ...
                # 某些错误不应该重试
                if error_code in [200671, 200341, 230002]:  # 权限错误, bot不在群中
                    return result
    ...
    async def send_markdown_message(
        ...
        """Send a message using Feishu interactive card with markdown rendering.

        This renders **bold**, # headers, `code`, tables, lists etc. in Feishu.
        """
        if not receive_id:
            receive_id = self.default_chat_id
        if not receive_id:
            print(f"[Feishu] Error: No receive_id for markdown message")
            return {"error": "No receive_id provided"}

        print(f"[Feishu] Sending markdown message to {receive_id[:10]}... (len={len(text)})")
        ...
        try:
            result = await self._make_request_with_retry("POST", url, headers, payload)
            print(f"[Feishu] Markdown message send result: {result}")
            return result
        except Exception as e:
            print(f"[Feishu] Error sending markdown message: {e}")
            return {"error": str(e)}
    ...
    async def send_long_text(
        ...
        """Send arbitrarily long text by splitting into sequential messages.

        Splits at natural boundaries: paragraphs → lines → characters.
        """
        if len(text) <= max_chars:
            result = await self.send_text_message(receive_id, text, receive_id_type)
        ...
        for idx, chunk in enumerate(chunks):
            header = ""
            if total > 1:
                header = f"*(Part {idx + 1}/{total})*\n\n"
            result = await self.send_text_message(receive_id, header + chunk, receive_id_type)
...
def build_progress_card(task_id: str, status: str, latest_output: str, tool_count: int = 0) -> dict:
    ...
    _CARD_MAX = 10000
    truncated_output = latest_output[:_CARD_MAX] if len(latest_output) > _CARD_MAX else latest_output
    if len(latest_output) > _CARD_MAX:
...
def build_result_card(
    ...
    if final_result:
        card["elements"].append(
            {
                "tag": "markdown",
                "content": "**最终结果:**\n" + final_result[:10000] + ("...\n\n📝 *(结果过长已截断，完整结果请查看终端或日志)*" if len(final_result) > 10000 else ""),
            }
        )
...
def build_dynamic_progress_card(
    ...
    if recent_output:
        _CARD_MAX = 10000
        truncated_output = recent_output[:_CARD_MAX] if len(recent_output) > _CARD_MAX else recent_output
        elements.append(
            {
                "tag": "markdown",
                "content": f"**📤 最近输出:**\n```\n{truncated_output}\n```{'...(已截断，完整结果请查看终端或日志)' if len(recent_output) > _CARD_MAX else ''}",
            }
        )
