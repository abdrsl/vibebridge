...
```bash
# 编辑配置文件
nano /home/user/workspace/vibebridge/config/commands.json
```
...
1. 检查配置文件是否存在：
   ```bash
   cat /home/user/workspace/vibebridge/config/commands.json
   ```
...
3. 查看日志：
   ```bash
   tail -f /home/user/workspace/vibebridge/logs/server.log | grep -E "(Command|指令)"
   ```
